...
>
> ⚠️ **One item still needs Rob before the affected workflow is built:** **Stale-bid threshold** — Jefferson says 3 days vs. prior 14; sets the Quote follow-up workflow timer (days a Vendor_Quote can sit without movement before it's flagged + the vendor is chased). Operationally Jefferson's call, but 3 days is aggressive for overseas vendors (time zones/holidays) and may generate noise — flag before building.
>
> ⏸️ **Property `2900` — ON HOLD per Kyle 06/04/26.** Not in the 8-property canon; do NOT build the picklist value. Revisit later; not a go-live blocker.
