...
13a. **[P1] 🔧 TODAY (06/05/26) — Connect the portal to live Zoho (read-only).** Runbook: `docs/PortalZohoConnect_Procurement_060526.md`. Build a Vercel serverless read-proxy (`/api/procurement.js`) + wire the portal's fetch, replacing mock data. **Confidence ~90%** (OAuth self-client/DC-domain friction + an access-guard decision are the risk). **Kyle (console):** create read-only OAuth self-client (`ZohoCRM.modules.READ`), exchange refresh token, set 6 Vercel env vars, decide access guard. **Claude (build):** the function + portal wiring (can start now, runs once env vars land). **DoD today:** portal renders the 3 live *test* records via the proxy on a protected Preview URL. ⛔ Real-data go-live stays gated (0 real records + operational hold).
