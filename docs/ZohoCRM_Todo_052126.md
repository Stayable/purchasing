...
13a. **[P1] ⏳ IN PROGRESS (06/05/26) — Connect the portal to live Zoho (read-only).** Runbook: `docs/PortalZohoConnect_Procurement_060526.md`. **Confidence ~90%.** Status:
   ...
   - 🔲 **P1-F — NEXT (Claude)** — wire the portal to `fetch('/api/procurement')` + render live data, with mock fallback + empty-state. Then P2-A verify on a protected Preview against the 3 test records.
