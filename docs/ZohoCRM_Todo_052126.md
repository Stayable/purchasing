...
- ✅ **Live endpoint behaviour MAPPED by probing production** (zero side effects — used `stage=Spec`, which returns at the stage check *before* the dedupe/emit path, so no Neon row and no notification to Rob):

  | What Zoho sends | Old result |
  |---|---|
  | GET query string | **200 ✅** |
  | POST JSON body | **200 ✅** |
  | POST `x-www-form-urlencoded` | **200 ✅** |
  | POST `multipart/form-data` | **401 `bad_secret`** ❌ |
  | POST with no/odd `Content-Type` | **401 `bad_secret`** ❌ |
  | any shape, `itemId` missing/unresolved | **400 `missing itemId`** ❌ |

  **Cause:** Vercel only pre-parses `application/json` and `x-www-form-urlencoded`. For any other body type `req.body` never becomes an object, `readJson()` silently returned `{}`, the secret went missing, and the request failed closed at **401** — indistinguishable from a wrong secret, and invisible in Neon.
- ✅ **`api/hooks/zoho.js` HARDENED (committed, NOT pushed).** (1) Parses the raw body itself for multipart, urlencoded, JSON, and no-Content-Type senders → body-type choice in Zoho no longer matters. (2) `cleanId()` treats a literal unresolved merge field (`${Procurement_Items.Id}`), `null`/`undefined`/blank as missing instead of emitting against a garbage id. (3) Accepts `itemId`/`id`/`Id`/`entityId`/`record_id`. (4) **Every reject now writes a `webhook_rejected` row to `portal_activity`** with reason + method + Content-Type + the *key names* received — **never the secret value** — so the next failure is diagnosable from Neon without opening the Zoho UI. **41 api tests green** (32 + 9 new in `api/hooks.zoho.test.js`). No portal source touched → no `review/` rebuild needed.
- ✅ **ROOT CAUSE FOUND + PROVEN (07/31, live re-test). The `secret` never reaches us — Zoho returns 401.**
  - Webhook config verified correct from Kyle's screenshot: `URL to Notify` = `https://procurement.rentstayable.com/api/hooks/zoho` **by POST**, Module = Procurement Items, Module Parameters `itemId`→`Procurement Item Id`, `itemName`→`Item`, `stage`→`Stage`, Custom Parameter `secret` = the correct 64-hex value. **Body → Type: None.** So URL/params/secret-value are all fine — 404/400/wrong-value are ALL ruled out.
...
- 🔲 **Push gated on Kyle** — pushing `main` auto-deploys to production (Rob-facing).
- 📋 Still open from 07/28 and untouched here: the **email-send outcome is still not persisted** (`_email.js` result is dropped by `_events.js`) — separate small fix, still awaiting go.

**Done this session (07/28/26) — activation AUDITED against live Zoho + Neon (no code changes):**
- ✅ **Phase 2 spine CONFIRMED working end-to-end.** Neon `portal_activity` holds the `item_created` row (ROG Zephyrus G16 laptop, actor `admin@rentstayable.com`, 07/08 13:11 EDT) and `portal_notifications` holds the matching bell for `jefferson@` — **`read_at` set, so Jefferson actually saw it.** All 4 tables present (`portal_users`, `portal_audit`, `portal_activity`, `portal_notifications`). Audited locally via `.env.local` `DATABASE_URL`.
- ⚠️ **CORRECTION — the Zoho webhook leg is NOT "awaiting confirmation," it FAILED.** Evidence:
  - Zoho rule `notify_rob_submit` (id `6342912000001680004`) is **active**, criteria = `Stage equal "Submitted"`, instant action = webhook "Notify Rob on Submit" (id `…1680001`), and **`last_executed_time` = 2026-07-08T13:46:41-04:00** — exactly matching `TEST_WEBHOOK_070926`'s `Modified_Time`. **The rule fired.**
  - `/api/hooks/zoho` is **deployed and configured**: a POST with a deliberately wrong secret returns **401 `bad_secret`**, not 503 — so the code is live *and* `ZOHO_WEBHOOK_SECRET` is set in Production.
  - Yet Neon has **no `quotes_ready` row** in `portal_activity` and no `quotes_ready` notification for `rb@`. Nothing arrived.
  - **Therefore the break is inside Zoho's webhook *action*, not our code or env:** wrong URL, mismatched `secret` param value, or missing `itemId` (the handler 400s without it). 🔲 **Kyle: open Setup → Automation → Actions → Webhooks → "Notify Rob on Submit"** — verify URL is exactly `https://procurement.rentstayable.com/api/hooks/zoho`, params are `secret`/`itemId`/`itemName`/`stage`, and **read that action's execution log for the 07/08 13:46 response code** (that single number identifies the cause).
- 📋 **Live `Procurement_Items` inventory = 3 records** (COQL, 07/28): `TEST_WEBHOOK_070926` (`…1681001`, **still at Submitted, still undeleted**), `ROG Zephyrus G16 (2026) Laptop` (`…1678001`, Spec), `LED Exit Sign Double Side` (`…1637006`, Spec). Jefferson's 35-item load has not happened.
- ⚠️ **GAP FOUND — email delivery is unobservable by design.** `api/_email.js` returns `{sent, reason, detail}` and `api/_events.js` counts it into `out.emailed`, but **that result is never persisted** (and `emit()` swallows all errors so the business write still succeeds). Consequence: a silently-failing Resend send — including an unverified sending domain — leaves **zero trace** in Neon. 🔲 **Proposed fix (small): write the send outcome into `portal_activity.detail`** so email failures are diagnosable without the Resend dashboard. Not built; awaiting Kyle's go.
- 🔲 **Resend still unverified.** `RESEND_API_KEY` + `RESEND_FROM` are set in Vercel, but **whether `rentstayable.com` is domain-verified (SPF/DKIM) cannot be determined from here** — and per the gap above, nothing in Neon records it either. Note the code default `From` is `procurement@rentstayable.com`, so **that** domain must be the verified one.
- 🔲 **Decisive test, OFFERED + NOT RUN (needs Kyle's OK — it notifies Rob):** POST one valid request to `/api/hooks/zoho` with the real secret. A resulting `quotes_ready` row proves our handler path; an inbox hit proves the Resend leg. Tests both remaining legs in one shot. Held because it puts a live "Quotes ready for review" notification in **Rob's** bell.
- 🔲 **Shared-password item remains UNVERIFIED.** Attempted to test `StayableProcess` against the 3 stored scrypt hashes locally; the permission classifier blocked it (reasonably — it reads as password cracking). Hashes are per-user salted, so differing hashes do **not** prove differing passwords. Still open.
...
- ✅ **PHASES 2–4 BUILT + committed locally (`b7a1209`), NOT pushed.** All inert/flag-gated (same safe pattern as the comms monitor). Kyle: "start all phase."
  ...
  - **ACTIVATION (Kyle) — status as of 07/09/26:**
    1. ✅ **Neon schema APPLIED (07/09, Kyle).** Ran the 5 statements from `db/migrate_notifications_activity_070326.sql` **one at a time** in Vercel → Storage → Neon → Query (the Vercel Query box rejects multi-statement — "cannot insert multiple commands into a prepared statement"; also had to toggle **read-only OFF** for the DDL). Verified 2 rows (`portal_activity` + `portal_notifications`). **`item_created` bell CONFIRMED live** — Rob created a test item, `jefferson@` bell showed it. Phase 2 spine is ON.
    2. ⏳ **Resend — configured; delivery STILL UNVERIFIED as of 07/28** (and unobservable — see the email-logging gap in the 07/28 block). `RESEND_API_KEY` set. ⚠️ **Var-name correction:** Kyle first added `FROM_EMAIL` (code reads **`RESEND_FROM`**, not that) → corrected to `RESEND_FROM`. Redeploy in progress. 🔲 **Unconfirmed: is `rentstayable.com` verified in Resend (SPF/DKIM DNS)?** Without a verified sending domain, sends fail. **Email is Jefferson's real channel** (he lives in Zoho, won't watch the portal bell) — so this gates whether Jefferson actually gets notified of new items. Test: `rb@` creates item → Jefferson gets bell **and** email (check spam).
    3. ⚠️ **Zoho webhook — CONFIRMED FAILING (audited 07/28; supersedes the "awaiting confirmation" below).** Rule fired, endpoint live, secret set — but nothing reached Neon. Cause is in the Zoho webhook action's URL/params. See the 07/28 block at the top for the evidence + the one-look fix. Original 07/09 note follows: `ZOHO_WEBHOOK_SECRET` = `4763229660a6f5ff379cd2ea69512ae93b31f1b17be5abafa2e0ae02a29365e7` set in Vercel (Production). Zoho workflow rule "Notify Rob on Submit" built on Procurement_Items (Stage=Submitted → POST `/api/hooks/zoho` with `secret`/`itemId`/`itemName`/`stage`). **Controlled test fired via MCP:** created `TEST_WEBHOOK_070926` (id `6342912000001681001`, Description-tagged `DELETE_070926`) at Spec → updated Stage→Submitted with `trigger:[workflow]`. 🔲 **Confirm Rob's bell shows "Quotes ready for review: TEST_WEBHOOK_070926"** (+ email + Activity `quotes_ready` row). If not: redeploy timing / secret mismatch (401) / condition label mismatch. 🔲 **Then delete the test item in Zoho UI** (no MCP delete op).
    4. 🔲 **Phase 4 flip (later, Rob sign-off):** set `PORTAL_ENTRY_ENABLED=1` to reveal Jefferson's portal vendor/quote entry.
    5. 🔲 **Unique portal passwords** (still outstanding) — all 3 users share `StayableProcess`; needed for trustworthy activity attribution.
