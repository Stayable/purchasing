...
0. **[P1]** Task 10 (v2) — Build the **`Vendor_Quotes` child module** on Procurement_Items (Kyle, manual UI) per `specs/ZohoModuleSpec_Quotes_060226.md`. **Build sequence:** (1) **Vendors** (native) — add 5 custom fields + `Vendor_Type`; migrate 21 vendor Accounts + 20 Contacts in. (2) **Procurement_Items** — add `Target_Qty`, add `Awarded_Vendor` (→ Vendor_Quotes), retire `Bid_Count` + Linked Vendors related list (`Winning_Vendor` deprecated). (3) **Vendor_Quotes** — create module (⚠️ **API name `Vendor_Quotes`, not `Quotes`** — stock Quotes occupies that name), fields, 7 picklists, 2 landed-cost formula fields. (4) Award-stamping **workflow rule** via `zoho-crm-workflows` MCP. (5) Grouped **landed-cost report** + **Kanban** on Quote Status. (6) Validate with a 3-quote test on one item. **When built, Kyle pings Kate/Claude** → bulk-load existing quotes/contacts via `updateRecords`/`createRecords`. *(`PO_Number`/`PO_Status` already built 05/29.)*
