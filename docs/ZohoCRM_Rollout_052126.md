...
| 07/01/26 | **Staying on Zoho for now — the 06/27 Neon in-app rebuild is DEFERRED, not active (Kyle)** | The 06/27 decision to rebuild the 3 procurement modules on Neon and retire Zoho (spec `docs/superpowers/specs/2026-06-27-portal-data-layer-design.md`) is parked. Zoho remains the live procurement store / system of record; the portal continues to read Zoho via `/api/procurement`. Rebuild spec stays as the documented future target (still DRAFT, still needs Rob sign-off on the operating-model change). No code or schema work toward the Neon migration proceeds under this direction. |
| 07/01/26 | **`Vendor_Quotes` record-name field split into Number + Name in live Zoho (Kyle, UI)** | The module's primary auto-number field (label was "Vendor Quote Name", API name `Name`, type autonumber, `QT-####`) was **relabeled "Vendor Quote Number"**; **API name kept as `Name`** so the live portal proxy (`api/procurement.js` `SELECT Name … FROM Vendor_Quotes`, mapped to `quotes[].name`) is unaffected — no code change, no production risk. A **new single-line text field "Vendor Quote Name" (API name `Quote_Name`)** was created for a free-text descriptive quote label (user-entered; `null` on existing records). Confirmed selectable via COQL. `Quote_Name` is **not yet surfaced in the portal** — wiring it requires adding it to the proxy COQL + quote table UI + a deploy (pending Kyle's go-ahead). This split also matches/answers open question #4 in the deferred Neon rebuild spec (split `quote_number` vs `quote_name`), already updated there. |
| 07/02/26 | **End-to-end portal procurement workflow approved for build on Zoho (Kyle)** | Loop: Rob creates item in the portal → notify Jefferson → **Jefferson stays in Zoho** (his stated preference) to add vendors/quotes → he sets Stage=Submitted in Zoho → a **Zoho workflow-rule webhook** (`/api/hooks/zoho`) notifies Rob → Rob reviews quotes+comms → approves/awards → notify Jefferson. Sub-decisions: email via **Resend** (`procurement@rentstayable.com`, new sending domain to verify); inline "create new vendor" from the quote form = **modal** (in-page state preserved) over a new tab; **activity log** captured to Neon and shown per-item + as a global feed; **roles = Rob creates+approves / Jefferson enters / Kyle admin-override only** (Kyle removed as a workflow actor), enforced server-side. The portal's vendor/quote entry forms are **built but hidden behind `PORTAL_ENTRY_ENABLED`** (default off) — revealed to Jefferson only if the process + UI prove out, which is itself a Rob-sign-off cutover. Interim activity-log gap acknowledged: Jefferson's Zoho-side vendor/quote adds live in Zoho's own history, not the portal log, until the future flip. Spec: `docs/superpowers/specs/2026-07-01-portal-procurement-workflow-design.md`. |
| 07/02/26 | **Phase 1 (portal create-item) built + committed (`568c285`), not yet pushed (Kyle + Claude)** | First slice of the 07/02 workflow. `POST /api/items` (session + role-gated to `rb@`/`admin@`) creates a `Procurement_Items` record at `Stage=Spec` via a new shared `api/_zohoWrite.js` helper (`getWriteToken` + `createRecord`, factored out of the award path's inline logic; award.js left untouched to avoid risk to the live approve path); logs an `item_created` row to Neon `portal_audit`. Portal UI: `canCreateItems` role helper, `createItem` client, `NewItemModal` form using the exact live Zoho Category + Property_Scope picklists (verified via `getFields` 07/02), and a role-gated **+ New item** button in the Items view. 3 api + 27 portal tests green; `review/` rebuilt. Plan: `docs/superpowers/plans/2026-07-02-portal-workflow-phase1-create-item.md`. **Not pushed** — pushing `main` auto-deploys the Rob-facing production site and the endpoint's prerequisites (session, role, write token) are all already configured, so it goes live immediately on push; held for Kyle's go-ahead + post-deploy verification. |
| 07/02/26 | **Phase 1 PUSHED live + verified (`f038667`)** | `git push origin main`; Vercel auto-deployed. Verified anon `POST /api/items` → 401 and anon `GET /api/procurement` → 401 (no regression). Browser spot-check (rb@ sees + New item, jefferson@ doesn't, create → Stage=Spec) left to Kyle. |
| 07/02/26 | **Phases 2–4 built + committed (`b7a1209`), not yet pushed — all inert/flag-gated (Kyle "start all phase" + Claude)** | Full remaining workflow, safe-inert like the comms monitor. **Phase 2 (notifications + activity):** Neon `portal_activity` + `portal_notifications` tables; `api/_db.js` CRUD + de-dupe; `api/_email.js` (Resend REST, inert without `RESEND_API_KEY`); `api/_events.js` single `emit()` hook so activity + webapp notification + email never drift (pure `recipientsFor`/`renderNotification` unit-tested); `api/notifications.js`, `api/activity.js`; `api/hooks/zoho.js` (secret-checked + de-duped webhook: Zoho workflow rule on Stage→Submitted → `quotes_ready` → notify Rob, so Jefferson never leaves Zoho). Frontend: `useNotifications`, `NotificationBell` (top bar + unread badge), `/activity` global feed + per-item `ActivityTimeline`. **Phase 3:** `api/award.js` emits `decision_made` → notify Jefferson. **Phase 4 (HIDDEN behind `PORTAL_ENTRY_ENABLED`, default off → 404):** `api/vendors.js` (GET/POST), `api/quotes.js`, `api/items/ready.js`; `procurement.js` surfaces `entryEnabled`; `canEnterData` helper; `VendorFormModal` + `QuoteFormModal` with inline vendor-add that preserves the quote draft (modal-on-modal). Field API names verified via `getFields` (Vendors: `Vendor_Name`/`Email`/`Vendor_Type`/`Country_of_Origin`; Vendor_Quotes: `Unit_Price`/`Order_Quantity`/`Freight_Cost`/`Duty_Tariff`/`Incoterm`/`Currency1`/`Lead_Time_days`/`Quote_Name`/`Spec_Match`/`Quote_Status`/`Date_Received` + `Procurement_Item`/`Vendor` lookups; landed cost is a Zoho formula). 32 api + 31 portal tests green; `review/` rebuilt. **Activation (Kyle, each independent):** (1) apply the new Neon schema — the one step that turns Phase 2/3 ON since `DATABASE_URL` is already set; (2) Resend domain + `RESEND_API_KEY` for email; (3) `ZOHO_WEBHOOK_SECRET` + the Zoho Stage→Submitted workflow rule; (4) `PORTAL_ENTRY_ENABLED=1` to flip Jefferson to portal entry (Rob sign-off); (5) unique portal passwords. |
